...
class N8nApiClient {
    ...
    async getAvailableNodes() {
        try {
            ...
            return response.data.data.map((node) => node.name);
        }
        catch (error) {
    ...
    async getCredentialTypes() {
        try {
            ...
            return response.data.data.map((cred) => cred.name);
        }
        catch (error) {
    ...
    async createWorkflow(workflow) {
        try {
            const response = await axios.post(`${N8N_API_URL}/workflows`, workflow, { headers: this.headers });
            return {
                id: response.data.id,
                success: true,
            };
    ...
    async activateWorkflow(id) {
        try {
            await axios.patch(`${N8N_API_URL}/workflows/${id}`, { active: true }, { headers: this.headers });
            return true;
        }
        catch (error) {
    ...
    async testWorkflow(id, testData) {
        try {
            const response = await axios.post(`${N8N_API_URL}/workflows/${id}/execute`, { workflowData: testData }, { headers: this.headers });
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            switch (name) {
                case 'validate-nodes': {
                    ...
                    const requestedNodes = args.nodes;
                    const missingNodes = requestedNodes.filter((node) => !availableNodes.includes(node));
                    return {
                        content: [
                            {
                                type: 'text',
                                text: JSON.stringify({
                                    ...
                                    availableNodes: availableNodes.filter((node) => requestedNodes.includes(node)),
                                    missingNodes,
                ...
                case 'validate-credentials': {
                    ...
                    const requestedCredentials = args.credentials;
                    const missingCredentials = requestedCredentials.filter((cred) => !availableCredentials.includes(cred));
                    return {
                        content: [
                            {
                                type: 'text',
                                text: JSON.stringify({
                                    ...
                                    availableCredentials: availableCredentials.filter((cred) => requestedCredentials.includes(cred)),
                                    missingCredentials,
                                }),
                            },
                        ],
                ...
                case 'enhanced-feasibility-check': {
                    ...
                    const requiredNodes = args.requiredNodes;
                    const requiredCredentials = args.requiredCredentials || [];
                    const availableNodes = await this.n8nClient.getAvailableNodes();
                    const availableCredentials = await this.n8nClient.getCredentialTypes();
                    const missingNodes = requiredNodes.filter((node) => !availableNodes.includes(node));
                    const missingCredentials = requiredCredentials.filter((cred) => !availableCredentials.includes(cred));
                        ...
                        content: [
                            {
                                ...
                                text: JSON.stringify({
                                    ...
                                    nodeCheck: {
                                        ...
                                        available: requiredNodes.filter((node) => availableNodes.includes(node)),
                                    },
                                    credentialCheck: {
                                        required: requiredCredentials,
                                        missing: missingCredentials,
                                        available: requiredCredentials.filter((cred) => availableCredentials.includes(cred)),
                                    },
